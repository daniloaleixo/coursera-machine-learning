...
For example when we have a classifier that is 0.5% of having cancer the error calculation could get messy. Because if we put all the non-cancer we would have 0.5% error.
So when we have a skewed class like this, we set:

![IMG](img/img8.png)

**Precision**: true positives / # predicted positives = true positive / (true positive + false positive)
**Recall**: true positives / # actual positives = true positive / (true positive + false negative)


###  Trading Off Precision and Recall

We would have to balance between precision and recall, for example we would make cancer-prediction more precise (higher precision), but it will gives a lower recall. On the other hand if we want to avoid missing too many cases of cancer (avoid false negatives), we would have a higher recall and lower precision

#### How to compare precision/recall numbers

We use F<sub>1</sub>Score: 

2 * <sup>PR</sup> / <sub>(P + R)</sub>
